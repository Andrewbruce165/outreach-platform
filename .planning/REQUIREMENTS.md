...
- **IMPT-03**: Per-account import — convert the vendor SQLite `.session` → `StringSession.save(SQLiteSession(path))` **offline** → `encrypt_session()` → `senders.session_string`; then `make_telegram_client` → `connect()` → `is_user_authorized()` → `get_me()` to populate `phone`/`telegram_id`/`tg_username`/`name`; disconnect immediately (D-11, D-12)
...
- **IMPT-05**: Store the 2FA password from the JSON `twoFA` field **Fernet-encrypted** (reuse `encrypt_session()`) in a new nullable `senders.twofa_password_enc` column; the plaintext is NEVER logged or returned in any API response (D-05, D-07)
- **IMPT-06**: Dedup by `telegram_id` — derived slug `sender-{tg_id}` already exists in the workspace → **skip + report** `already_connected`, existing session is NOT overwritten (mirrors CSV skip+report, Phase 2 D-03); proxy = JSON `proxy` if set, else a free `ProxyPool` entry (D-14, D-15)
- **IMPT-07**: **Per-file partial success** — each pair resolves independently to `ok` | `failed` (+ reason); a broken/unpaired/invalid pair NEVER fails the whole batch; successfully imported accounts start `lifecycle_status='active'` + `restriction_status='none'` (no @SpamBot probe at import — reconcile picks up restrictions later) (D-10, D-13)
