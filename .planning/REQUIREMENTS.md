...
- [ ] **INBM-04**: Incoming media recording — listener `NewMessage` handler tags each incoming photo/video/voice/document with a concrete `message_type` + `file_name`/`mime_type`/`size_bytes` read pre-download; bytes NOT fetched at receipt (D-15); voice still transcribed + AI-dispatched; idempotent on duplicate `telegram_message_id`
...
- [ ] **INBM-06**: Structured error codes — `MESSAGE_EDIT_TOO_OLD`, `MESSAGE_NOT_EDITABLE`, `DELETE_FAILED`, `FILE_TOO_LARGE`, `MEDIA_UNAVAILABLE`, `MESSAGE_NOT_FOUND` + reused `NO_TELEGRAM_ID`/`RECIPIENT_NOT_IN_TELEGRAM`/`FLOOD_WAIT`/`ACCOUNT_FROZEN`/`USER_IS_BLOCKED`; mapped via a Phase-23 clone of `_raise_profile_telegram_error` (D-17)
- [ ] **INBM-07**: REST-by-message_id API + workspace gate — all four endpoints under `/api/v1/conversations` with `Depends(auth_dep)`, workspace-scoped via `_load_conversation_or_404`/a joined SELECT; cross-workspace or wrong-conversation `message_id` → opaque 404; message must belong to the conversation+workspace (D-18, D-19)
