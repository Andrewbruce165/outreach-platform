...
- [ ] **INBM-08**: `messages` schema extension — idempotent migration `053_phase23_messages_media.sql`: `message_type VARCHAR(20) NOT NULL DEFAULT 'text'` + CHECK(`text|photo|video|voice|document`), `file_name`/`mime_type`/`size_bytes` nullable, `edited_at TIMESTAMPTZ` nullable, `message_text` DROP NOT NULL, NO `deleted_at`; `messages` has NO ORM model (raw-SQL only) so the DB DEFAULT is the sole default source — do NOT add a Message ORM class; `tests/conftest.py` hardcoded migration list gains an exists-guarded 053 apply (D-20, D-21)
