...
- **IMPT-04**: Capture the per-account **client fingerprint** from the vendor JSON (`device`→`device_model`, `sdk`→`system_version`, `app_version`→`app_version`, `lang_code`, `system_lang_code`; `lang_pack` stays `"tdesktop"`) into `senders.client_fingerprint`; `make_telegram_client`/`get_client` gain an optional `fingerprint` override with a **strict NULL fallback** (NULL → byte-identical to today's global `_CLIENT_FINGERPRINT` — no regression to the working 13); `api_id`/`api_hash` stay **global**, NOT per-account (D-01, D-02, D-03, D-04)
