...
## Phase 21: Bulk Telegram account import via session JSON upload in UI (IMPT-01..10)

*Derived during /gsd:plan-phase 21 from 21-CONTEXT.md decisions D-01..D-17 (see also 21-RESEARCH.md §Phase Requirements and 21-NOTES.md grounded vendor-file analysis). Phase adds ONE migration (051). Import bypasses phone/SMS onboarding by converting a live vendor Telethon SQLite `.session` into our encrypted StringSession storage.*

- [x] **IMPT-01**: ZIP upload **preview** endpoint `POST /api/v1/accounts/import/preview` (multipart) — unzip in-memory, pair `<base>.json`↔`<base>.session` by basename (`session_file` field), validate each JSON against the vendor Pydantic schema, **NO Telegram connect**; stage the raw ZIP bytes in `account_import_stagings` (BYTEA + `expires_at` TTL, mirrors `csv_imports`); return `{ import_id, matched[], unpaired[], malformed[] }`; ZIP-safety guards (byte cap, max accounts, basename-only member names, reject `..`/absolute paths) (D-08, D-08a, D-10)
- [x] **IMPT-02**: **Confirm** endpoint `POST /api/v1/accounts/import/{import_id}/confirm { role }` — validates `role ∈ {sender,checker}` (chosen ONCE for the whole batch, D-16), re-reads the staged ZIP (410 if expired), creates ONE `account_import_jobs` row + N `account_import_items` rows (`status='pending'`), returns `{ job_id }` (202) immediately; `AccountImportWorker` drives each item pending→processing→ok/failed; `GET /api/v1/accounts/import/{job_id}/status` reports `{ processed, total, items[] }` (D-09, D-16, D-10)
- [x] **IMPT-03**: Per-account import — convert the vendor SQLite `.session` → `StringSession.save(SQLiteSession(path))` **offline** → `encrypt_session()` → `senders.session_string`; then `make_telegram_client` → `connect()` → `is_user_authorized()` → `get_me()` to populate `phone`/`telegram_id`/`tg_username`/`name`; disconnect immediately (D-11, D-12)
- [x] **IMPT-04**: Capture the per-account **client fingerprint** from the vendor JSON (`device`→`device_model`, `sdk`→`system_version`, `app_version`→`app_version`, `lang_code`, `system_lang_code`; `lang_pack` stays `"tdesktop"`) into `senders.client_fingerprint`; `make_telegram_client`/`get_client` gain an optional `fingerprint` override with a **strict NULL fallback** (NULL → byte-identical to today's global `_CLIENT_FINGERPRINT` — no regression to the working 13); `api_id`/`api_hash` stay **global**, NOT per-account (D-01, D-02, D-03, D-04)
- [x] **IMPT-05**: Store the 2FA password from the JSON `twoFA` field **Fernet-encrypted** (reuse `encrypt_session()`) in a new nullable `senders.twofa_password_enc` column; the plaintext is NEVER logged or returned in any API response (D-05, D-07)
- [x] **IMPT-06**: Dedup by `telegram_id` — derived slug `sender-{tg_id}` already exists in the workspace → **skip + report** `already_connected`, existing session is NOT overwritten (mirrors CSV skip+report, Phase 2 D-03); proxy = JSON `proxy` if set, else a free `ProxyPool` entry (D-14, D-15)
- [x] **IMPT-07**: **Per-file partial success** — each pair resolves independently to `ok` | `failed` (+ reason); a broken/unpaired/invalid pair NEVER fails the whole batch; successfully imported accounts start `lifecycle_status='active'` + `restriction_status='none'` (no @SpamBot probe at import — reconcile picks up restrictions later) (D-10, D-13)
- [x] **IMPT-08**: Idempotent migration `051_account_import.sql` (auto-applied): `senders.client_fingerprint JSONB NULL` + `senders.twofa_password_enc TEXT NULL` + `account_import_stagings` + `account_import_jobs` + `account_import_items` tables; ORM mirrors (`Sender` 2 cols + 3 new model classes) with `server_default` on every NOT NULL column and `gen_random_uuid()` id defaults (create_all builds the test/fresh-DB schema from the ORM — memory `project-orm-default-vs-server-default-drift`) (D-01, D-05, D-08a, D-09)
- [x] **IMPT-09**: Frontend two-step bulk-import UI (sibling repo `aimly-tg-outreach`): ZIP upload → preview summary (matched/unpaired/malformed) → role radio (sender/checker) → confirm → progress poll with per-account result rows; regenerate `lovable-handoff/openapi.json` + types via export-handoff (rebuild api first), no manual spec editing; human-UAT (D-08, D-08a, D-09, D-16)
- [x] **IMPT-10**: Phase 20's 2FA-change endpoint (`POST /senders/{slug}/2fa`), for imported accounts, uses the stored (server-side-decrypted) `twofa_password_enc` as the `current_password` when the request omits it — the user does not re-type it; the plaintext stays server-side and is NEVER returned (reconciles D-06 auto-fill with D-07 never-return) (D-06)

**Deviation note:** D-05/IMPT-05 deliberately deviates from Phase 20 D-03 (which forbids persisting the 2FA password), scoped to **imported accounts only** — bulk import of hundreds of purchased accounts makes manual per-account 2FA entry unworkable, and the client wants the platform to retrieve it when needed. Encryption-at-rest is mandatory; blast-radius trade-off accepted (the DB already holds `session_string` = full account access encrypted).

**Plan → requirement map:** 21-01 → IMPT-08 (+Wave-0 scaffold for all); 21-02 → IMPT-04, IMPT-10; 21-03 → IMPT-01; 21-04 → IMPT-03, IMPT-05, IMPT-06, IMPT-07; 21-05 → IMPT-02, IMPT-07; 21-06 → IMPT-09.

*2026-07-06 — derived IMPT-01..10 (Bulk Telegram account import) during Phase 21 planning.*

---

## Phase 23: Edit and delete-for-everyone of sent messages + file sending from inbox UI (INBM-01..09)

*Derived during /gsd:plan-phase 23 from 23-CONTEXT.md decisions D-01..D-22 (see also 23-RESEARCH.md §Phase Requirements). No REQ-IDs were pre-assigned (ROADMAP mapped them TBD); the planner derived the INBM-* cluster from the 22 locked decisions per the established NDLG/PACE/WARM/KB/SRLD/LLMP/PROF/IMPT pattern. Phase adds ONE migration (053). All operations are OUT-OF-QUEUE direct Telethon calls (not queued sends) — queue intervals/rate-limits untouched.*

- [x] **INBM-01**: Delete-for-everyone — `DELETE /conversations/{id}/messages/{message_id}` revokes an OUTBOUND message (`revoke=True`, D-02) then HARD-deletes the `messages` row (D-03, no tombstone); list preview recomputes from the existing LATERAL subquery; NO auto-takeover (D-04); a stale/own-message revoke is a silent Telegram no-op so `DELETE_FAILED` is reserved for connection/flood/frozen failures (D-01)
- [x] **INBM-02**: Edit sent text — `PATCH /conversations/{id}/messages/{message_id}` edits an OUTBOUND text message (`edit_message`, D-06), updates `message_text` in place + sets `edited_at=NOW()` (D-07, no version history, UI shows «(изменено)»); text-only (D-05); NO auto-takeover (D-08); `MessageNotModifiedError`→success no-op, edit-window catch-and-map (D-17)
- [x] **INBM-03**: Send file from inbox — `POST /conversations/{id}/send-file` (multipart→temp→Telethon, D-09), ≤50 MB streamed with early 413 `FILE_TOO_LARGE` abort (D-10), auto-media `force_document=False` (D-11), NEW outbound → auto-takeover like `/send` (`status='manual'`, `ai_enabled=false`, `paused_reason`, cancel pending queue for `recipient_phone`, D-12), caption overflow >1024 → follow-up text (D-13), `/send` gates ported (active+ok sender, `NO_TELEGRAM_ID`, peer by telegram_id, temp file unlinked, bytes never in DB, D-14)
- [x] **INBM-04**: Incoming media recording — listener `NewMessage` handler tags each incoming photo/video/voice/document with a concrete `message_type` + `file_name`/`mime_type`/`size_bytes` read pre-download; bytes NOT fetched at receipt (D-15); voice still transcribed + AI-dispatched; idempotent on duplicate `telegram_message_id`
- [x] **INBM-05**: Lazy media download — `GET /conversations/{id}/messages/{message_id}/file` fetches an incoming file's bytes on demand from Telegram by `(peer, telegram_message_id)` (never `file.id`), streams `Response` with mime + `Content-Disposition` (attachment default, `?disposition=inline` opt); deleted-on-Telegram → 410 `MEDIA_UNAVAILABLE`; bytes never persisted (D-16, D-18)
- [x] **INBM-06**: Structured error codes — `MESSAGE_EDIT_TOO_OLD`, `MESSAGE_NOT_EDITABLE`, `DELETE_FAILED`, `FILE_TOO_LARGE`, `MEDIA_UNAVAILABLE`, `MESSAGE_NOT_FOUND` + reused `NO_TELEGRAM_ID`/`RECIPIENT_NOT_IN_TELEGRAM`/`FLOOD_WAIT`/`ACCOUNT_FROZEN`/`USER_IS_BLOCKED`; mapped via a Phase-23 clone of `_raise_profile_telegram_error` (D-17)
- [x] **INBM-07**: REST-by-message_id API + workspace gate — all four endpoints under `/api/v1/conversations` with `Depends(auth_dep)`, workspace-scoped via `_load_conversation_or_404`/a joined SELECT; cross-workspace or wrong-conversation `message_id` → opaque 404; message must belong to the conversation+workspace (D-18, D-19)
- [x] **INBM-08**: `messages` schema extension — idempotent migration `053_phase23_messages_media.sql`: `message_type VARCHAR(20) NOT NULL DEFAULT 'text'` + CHECK(`text|photo|video|voice|document`), `file_name`/`mime_type`/`size_bytes` nullable, `edited_at TIMESTAMPTZ` nullable, `message_text` DROP NOT NULL, NO `deleted_at`; `messages` has NO ORM model (raw-SQL only) so the DB DEFAULT is the sole default source — do NOT add a Message ORM class; `tests/conftest.py` hardcoded migration list gains an exists-guarded 053 apply (D-20, D-21)
- [x] **INBM-09**: Frontend handoff — regenerate `lovable-handoff/openapi.json` (4 new endpoints + `MessageResponse` extension + `EditMessageRequest`/`SendFileFromUIResponse`) via export-handoff (no hand-editing) + document D-17 codes in `error-codes.md`; multipart/new bodies tolerate Lovable field aliases; Lovable regenerates the sibling repo UI (D-22)

**Plan → requirement map:** 23-01 → INBM-08 (+Wave-0 RED scaffold for INBM-01..07); 23-02 → INBM-01, INBM-02, INBM-03, INBM-05, INBM-06 (Telethon service methods); 23-03 → INBM-01, INBM-02, INBM-06, INBM-07 (edit/delete endpoints, no-takeover); 23-04 → INBM-04 (listener incoming media); 23-05 → INBM-03, INBM-05, INBM-07 (send-file/download endpoints); 23-06 → INBM-09 (handoff + live-smoke).

**Behavioural invariant (the heart of the phase):** NEW outbound (text/file send) → auto-takeover (status='manual', ai_enabled=false, cancel pending queue); EDIT/DELETE of PAST messages → NO takeover.

*2026-07-07 — derived INBM-01..09 (Edit/delete/file-send + incoming media inbox mutations) during Phase 23 planning.*
