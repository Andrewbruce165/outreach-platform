...
<task type="auto">
  ...
  <action>
    ...
    - `_tick` has a TWO-LEVEL query (VERIFIED against contact_check_worker.py 246-311): an OUTER SELECT (~246-253) projects `s.id AS checker_id, s.slug AS checker_slug, s.session_string, s.proxy` from the derived table `s`, and an INNER `JOIN LATERAL (SELECT id, slug, session_string, proxy FROM senders ...) s` (~256). `first` (from `rows = result.fetchall()`) is an OUTER Row, so `first.client_fingerprint` resolves ONLY if the OUTER projection carries the column — adding it to the inner subquery alone leaves it invisible to the outer Row, and `first.client_fingerprint` then raises AttributeError on EVERY tick (a regression to the whole working checker pool — D-02, D-17). Therefore edit BOTH levels:
        (1) INNER LATERAL subquery SELECT (~256): `SELECT id, slug, session_string, proxy` → `SELECT id, slug, session_string, proxy, client_fingerprint`.
        (2) OUTER derived-table projection (~253, right after `s.proxy`): add `s.client_fingerprint` (leave it unaliased to match the unaliased `s.session_string`/`s.proxy`, so the Row attribute is exactly `client_fingerprint`).
      Then in the `common = dict(...)` (~351) add `fingerprint=first.client_fingerprint` (threads into both `check_phones(**common)` and `check_usernames(**common)`). NB: `probe_checker` (~609) and `_recover_checkers` (~809) are FLAT single-level SELECTs straight from `senders` — the added column reaches their Row directly, so they have NO outer projection to touch (do them exactly as the two bullets below say).
  ...
  <acceptance_criteria>
    ...
    - `grep -c "client_fingerprint" app/services/contact_check_worker.py` returns >= 6 (the inner LATERAL SELECT AND the outer projection in `_tick`, the probe_checker SELECT, the _recover_checkers SELECT, the `common` dict line, and the two probe_control call sites) — but the grep count ALONE cannot catch the outer-vs-inner bug, so it is NOT sufficient on its own
    - `grep -q "s.client_fingerprint" app/services/contact_check_worker.py` succeeds — proves the OUTER `_tick` projection (~253) carries the column, not just the inner LATERAL subquery; this is the exact line that makes `first.client_fingerprint` resolvable on the outer Row
    - `grep -q "fingerprint=first.client_fingerprint" app/services/contact_check_worker.py` succeeds AND `grep -c "fingerprint=row.client_fingerprint\|fingerprint=r.client_fingerprint" app/services/contact_check_worker.py` >= 2 succeed
    - REAL runtime assertion the weak grep cannot give — the existing `_tick`-driving suite runs GREEN under the test-overlay: `docker compose -f docker-compose.yml -f docker-compose.test.yml run --rm api pytest tests/test_contact_check_worker.py -q`. These tests call `await worker._tick()` end-to-end through the batch-resolve path that reads `first.client_fingerprint`; if the OUTER SELECT omits the column they raise AttributeError and the suite goes RED (the assertion that would have caught this blocker)
