...
### UI — вид Knowledge Base (на двух уровнях)
Уровень детализации статуса **зафиксирован** (раньше был на усмотрение — теперь решён по приложенному примеру). KB detail-вид показывает данные **и по всей базе, и по каждому документу**.

- **D-09 (агрегат по всей KB):** в шапке KB detail-вида — счётчики `DOCUMENTS`, `INDEXED`, `PROCESSING`, `FAILED`, объём `STORAGE` (в байтах), общий `Status` базы (напр. `READY`), `Type` (напр. `Files`) и `Updated` (дата/время последнего обновления). Это сводка по выбранной KB целиком.
- **D-10 (по каждому документу):** таб **Documents** показывает список загруженных документов с данными по каждому — статус индексации (indexed / processing / failed), размер, дата. Пользователь видит, какой именно файл упал/индексируется.
- **D-11 (навигация KB detail):** табы внутри вида KB — **Documents** (список/управление документами), **Search** (тест-поиск по KB прямо из UI — проверить, что находится), **Agents** (к каким агентам подключена эта KB — обратная сторона M:N из D-07), **Settings** (настройки KB). Точная раскладка/поля — за UI-фазой; набор табов и два уровня статистики (KB-агрегат + per-document) — locked.
- **D-12 (тема):** KB-экраны рендерятся в **существующей светлой теме** приложения (Telegram-blue, токены `aimly.css`) — подтверждено пользователем 2026-06-30. Приложенный скрин был тёмным, но взят **только как референс раскладки/IA**, не темы. Dark-режим на всё приложение — отдельная фаза (Deferred). Деталь зафиксирована в `16-UI-SPEC.md` (theme-reconciliation note).
...
- Точная визуальная раскладка KB-вью, поля таба Settings, поведение таба Search (порог/кол-во результатов в тест-поиске) — детализируется в `/gsd:ui-phase 16`.
</decisions>
...
- **Референс KB detail-вида (приложен пользователем 2026-06-30):** шапка со сводкой `Type: Files` · `Status: READY` · `Updated …` и строкой счётчиков `DOCUMENTS / INDEXED / PROCESSING / FAILED / STORAGE`; ниже табы `Documents · Search · Agents · Settings`. Задаёт ожидаемый вид (D-09/D-10/D-11). `Type: Files` соответствует v1-источникам (D-01) — поле типа оставляет место под будущие источники (напр. URL), но в v1 — только Files.
</specifics>
...
- **Dark-режим приложения** — KB (и весь UI) сейчас светлые (D-12). Полноценная тёмная тема (как на скрине-референсе) — отдельная фаза на всё приложение, не Phase 16.
