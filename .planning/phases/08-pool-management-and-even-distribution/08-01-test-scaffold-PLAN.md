...
<task type="auto">
  ...
  <action>
    ...
    - `test_rebalance_evens_cold_pending` (POOL-07): running campaign, sender A holds N cold-pending rows (seed via test_queue_item_factory), sender B newly attached → after `rebalance_on_attach(campaign_id, B_id, db)` assert the **newly-attached sender B** holds a pending count within ±1 of `total/P` via `SELECT sender_id, COUNT(*) FROM message_queue WHERE campaign_id=:cid AND status='pending' GROUP BY sender_id` (assert on B's bucket specifically). Rationale: D-08 intends a one-directional back-fill of the new account, so the ±1 guarantee is scoped to the new sender — the algorithm does NOT re-even pre-existing donors against each other, and a single pass caps at BATCH_CAP=500. Use a 2-sender pool (P=2) here so the back-fill of B necessarily evens the whole pool; do NOT assert whole-pool ±1 for P≥3 uneven donors (out of v1 scope, see 08-02 note).
