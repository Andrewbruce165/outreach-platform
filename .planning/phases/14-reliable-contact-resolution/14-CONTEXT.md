...
### Пул чекеров
- **D-03:** Пул из **2–3 выделенных** checker-аккаунтов (`role='checker'`), не участвующих в отправке — throttle бьёт только по резолву, не по кампаниям. **Статус (2026-06-26): пул уже частично провизионен** — 2 доказанно здоровых резолвера переведены `sender → checker` и **запаркованы** (`auth_status='restricted'`, `lifecycle_status='paused'`, `restriction_status='none'`): `sender-7979031303` (резолвил обоими методами в `diag_resolve.py`) и `sender-8364639216` (48/49 в `known_live_probe.py`). Запаркованы намеренно — текущий бескаповый воркер их не берёт; **активировать** (`auth_status='ok'`, `lifecycle_status='active'`) **только когда кап/probe/ротация будут в коде**. Третий `role='checker'` — старый битый `sender-8428118140` — оставить запаркованным (реальный shadow-ban). Откат конверсии: вернуть `role='sender', auth_status='ok', lifecycle_status='active'`.
...
- ~~Онбординг 2–3 checker-аккаунтов~~ — **сделано 2026-06-26**: 2 сендера переведены в запаркованные чекеры (см. D-03). Онбординг новых аккаунтов больше не блокер; planner'у остаётся лишь шаг **активации** этих двух при готовой защите.
