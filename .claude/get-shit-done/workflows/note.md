...
- **Global scope**: `/root/apps/telegram-api/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/root/apps/telegram-api/.claude/notes/`)
...
2. Glob `/root/apps/telegram-api/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/root/apps/telegram-api/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/root/apps/telegram-api/.claude/notes/` — works in any directory
