...
# === Campaign detail redesign: Logs + attachment metadata =====================

# Event types surfaced by GET /campaigns/{id}/logs. Two MVP sources, no new
# tables (redesign brief): message_queue outcomes + llm_calls.tool_calls
# built-in triggers. 'dialog_finished' (not 'finished') — namespaced away from
# the campaign-level status of the same name.
CAMPAIGN_LOG_EVENT_TYPES = {
    "message_queued",     # queue row still pending/processing
    "message_sent",       # queue row status='sent'
    "message_failed",     # queue row status='failed' (detail = error_message)
    "message_cancelled",  # queue row status='cancelled'
    "lead",               # llm tool_call mark_as_lead
    "handoff",            # llm tool_call transfer_to_manager
    "dialog_finished",    # llm tool_call finish_conversation
}


class CampaignLogEvent(BaseModel):
    """One chronological event of a campaign (newest-first in the response)."""

    ts: datetime
    type: str  # one of CAMPAIGN_LOG_EVENT_TYPES
    contact_name: Optional[str] = None
    contact_phone: Optional[str] = None
    conversation_id: Optional[UUID] = None
    # message_failed → error_message; llm events → None (payload stays in DB).
    detail: Optional[str] = None


class CampaignLogsResponse(BaseModel):
    """GET /campaigns/{id}/logs — cursor-paginated union of queue + llm events.

    next_before: pass back as ?before= to fetch the next (older) page;
    None when this page exhausted the history.
    """

    events: List[CampaignLogEvent]
    next_before: Optional[datetime] = None


class CampaignAttachmentItem(BaseModel):
    """One stored attachment echoed by POST /campaigns/{id}/attachment."""
...
class CampaignAttachmentUploadResponse(BaseModel):
    """POST /campaigns/{id}/attachment response (260709-dbl album upload).
    ...
    campaign_id: str
    count: int
    attachments: List[CampaignAttachmentItem]
    file_name: str
    size_bytes: int
    content_type: Optional[str] = None


class CampaignAttachmentMeta(BaseModel):
    """One first-message attachment (metadata only — bytes served separately
...
class CampaignAttachmentsResponse(BaseModel):
    attachments: List[CampaignAttachmentMeta]
