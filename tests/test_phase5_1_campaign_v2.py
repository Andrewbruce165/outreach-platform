"""Pydantic schema tests for Campaign v2 + Phase 11 field split (UI-CAMPB-01, schema layer).

Pure schema validation — no DB hit. Covers CampaignCreate/Update v2/Phase-11 fields
+ ToolSpec relaxation (Pitfall 6 — per-tool webhook_url is now Optional).

Phase 11 D-13: success_criteria removed from CampaignCreate/Update.
...
def test_campaign_create_with_v2_columns():
    c = CampaignCreate(
        ...
        lead_trigger_hint="They confirm a calendar slot",
    ...
    # Phase 11 D-13: success_criteria must NOT exist
    assert not hasattr(c, "success_criteria")
...
def test_campaign_create_phase11_dialogue_flow():
    """Phase 11 D-04: dialogue_flow validated as list of DialogueStage (max 7)."""
    c = CampaignCreate(
        name="DF test",
        message_template="hi",
        dialogue_flow=[
            DialogueStage(title="Intro", instruction="Greet the prospect"),
            DialogueStage(instruction="Ask about their needs"),
        ],
    )
    assert len(c.dialogue_flow) == 2
    assert c.dialogue_flow[0].title == "Intro"
    assert c.dialogue_flow[1].title is None


def test_campaign_create_phase11_dialogue_flow_max_7():
    """Phase 11 T2: dialogue_flow list capped at 7 stages."""
    stages = [DialogueStage(instruction=f"Stage {i}") for i in range(7)]
    c = CampaignCreate(name="DF max", message_template="hi", dialogue_flow=stages)
    assert len(c.dialogue_flow) == 7

    # 8 stages must fail validation
    with pytest.raises(ValidationError):
        CampaignCreate(
            name="DF overflow",
            message_template="hi",
            dialogue_flow=[DialogueStage(instruction=f"Stage {i}") for i in range(8)],
        )


def test_campaign_create_phase11_arguments_and_rules():
    """Phase 11 D-12/D-14: arguments_facts and campaign_rules accepted."""
    c = CampaignCreate(
        name="Args test",
        message_template="hi",
        arguments_facts="Saves 10h/week per SDR.",
        campaign_rules="Never mention competitors.",
    )
    assert c.arguments_facts == "Saves 10h/week per SDR."
    assert c.campaign_rules == "Never mention competitors."
